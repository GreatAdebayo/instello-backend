...
export class SignupController {
    ...
    async defaultSignup(@Body() body: UserDto, @Res() res, @Req() req) {
        ...
        const response: ResponseDto = await this.signupService.defaultSignup(body, ip)
    ...
    async verify(@Body() body: VerifyDto, @Res() res) {
        const response: ResponseDto = await this.signupService.verify(body)
        return res.status(response.status).json(response)
    }
    ...
    async resendCode(@Body() body: SendCodeDto, @Res() res) {
        const response: ResponseDto = await this.signupService.resendCode(body)
        return res.status(response.status).json(response)
    }
}
